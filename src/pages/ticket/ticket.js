...
axios({ method: "GET", url: "ticket.json", }).then((res) => {
	if (res.status == 200) {
		...
		newsList.forEach((news, i) => $append($newsList, "div").innerHTML = `<p><span>${i}</span><span>${news.title}</span><span>${news.year}/${news.month}/${news.date}</span></p><br />`);
		...
		function olList(HTMLElement, ticket) {
			$html($append(HTMLElement, "li"), ticket);
		}
		result.Visit_time.forEach((time, i) => $append($all("#Visit>tbody")[0], "tr").innerHTML = `<th>${i == 0 ? "旺季" : "淡季"}（每年${time.start}至${time.end}）</th><td>${time.Start_selling_tickets}</td><td>${time.Stop_selling_tickets}</td><td>${time.Stop_to_enter}</td><td>${time.Scenic_area_closed}</td>`);
		result.line.forEach(line => $append($id("line"), "li").innerHTML = line);
		result.policy.forEach(p => {
			let li = $append($id("policy"), "li");
			li.innerHTML = `<h3>${p.title}</h3>`;
			let ol = $append(li, "ol");
			ol.className = "ordered_list";
			p.texts.forEach(text => $append(ol, "li").innerHTML = text);
		});
