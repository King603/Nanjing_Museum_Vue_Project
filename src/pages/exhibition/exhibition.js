...
let bgImgUrl = "../static/bg/exhibition_bg.jpg";
let logoImg = "../static/陈列展览.jpg";
...
/**
 * 
 * @param {{ title: string; isShow: boolean; menu: { title: string; isShow?: boolean; menu?: { title: string; }[]; }[];}} menu 
 * @param {string} className 
 */
function setLiTitle(menu, className) {
	let li = $append($menu, "li");
...
/**
 * 
 * @param {{ menu: { title: string; isShow?: boolean; menu?: { title: string; }[]; }[];}} param0 
 * @param {string} className 
 * @param {(menu: { title: string; isShow?: boolean; menu?: { title: string; }[]; }) => void} callBack 
 */
function createLi({ menu }, className, callBack) {
	menu.forEach(menu => {
...
let $text = $id("text");
texts.forEach(text => $html($append($text, "p"), `${text}<br /><br />`));
